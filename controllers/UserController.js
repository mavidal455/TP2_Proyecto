...
class UserController {
  ...
  getAllUserController = async (req, res) => {
    ...
    try {
      ...
      if (!users) {
        res.status(204).json("No hay usuarios en el servidor");
      }
  ...
  postUserController = async (req, res) => {
    try {
      ...
      res.status(200).json({ success: true, message: nuevoUser });
    } catch (error) {
      res.status(500).send("Error del servidor");
    }
  };
  ...
  putUserController = async (req, res) => {
    try {
      ...
      res.status(200).json({ success: true, message: userModificado });
    } catch (error) {
  ...
  deleteUserController = async (req,res) => {
    try {
      ...
      if (!usuarioEliminado) {
        res.status(204).json("No se pudo eliminar el usuario");
      }

      res.status(200).json({ success: true, message: usuarioEliminado });
    } catch (error) {
      res.status(500).json("Error del servidor");
    } 
  }
